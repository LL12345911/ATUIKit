...
- (BOOL)qmui_isHeaderPinnedForSection:(NSInteger)section {
    ...
    BOOL isSectionScrollIntoContentInsetTop = self.contentOffset.y + self.at_contentInset.top > CGRectGetMinY(rectForSection);// 表示这个 section 已经往上滚动，超过 contentInset.top 那条线了
    BOOL isSectionStayInContentInsetTop = self.contentOffset.y + self.at_contentInset.top <= CGRectGetMaxY(rectForSection) - CGRectGetHeight(rectForHeader);// 表示这个 section 还没被完全滚走
...
- (BOOL)qmui_isHeaderVisibleForSection:(NSInteger)section {
    ...
    BOOL isSectionStayInContentInsetTop = self.contentOffset.y + self.at_contentInset.top < CGRectGetMaxY(rectForSection);// 表示这个 section 还没被完全滚走
...
- (void)qmui_scrollToRowFittingOffsetY:(CGFloat)offsetY atIndexPath:(NSIndexPath *)indexPath animated:(BOOL)animated {
    ...
    if (canScrollRowToTop) {
        ...
        [self at_scrollToBottomAnimated:animated];
    }
}
...
- (BOOL)qmui_canScroll {
    ...
    if ([self.tableHeaderView isKindOfClass:[UISearchBar class]]) {
        BOOL canScroll = self.qmui_realContentSize.height + UIEdgeInsetsGetVerticalValue(self.at_contentInset) > CGRectGetHeight(self.bounds);
        return canScroll;
    } else {
