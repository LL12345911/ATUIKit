...
@property (nonatomic, strong) NSMutableArray *touchLocations;
@property (nonatomic, strong) NSMutableArray *touchWords;
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _touchLocations = [[NSMutableArray alloc] init];
        _touchWords = [[NSMutableArray alloc] init];
...
- (void)drawTextInRect:(CGRect)rect
{
    ...
    for (NSString *word in words)
    {
        ...
        if (![wordCharacters isEqualToString:@""])
        {
            ...
            [_touchWords addObject:wordCharacters];
            [_touchLocations addObject:[NSValue valueWithCGRect:CGRectMake(drawPoint.x, drawPoint.y, sizeWordCharacters.width, sizeWordCharacters.height)]];
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    [_touchLocations enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        CGRect touchZone = [obj CGRectValue];

        if (CGRectContainsPoint(touchZone, touchPoint)) {
            //A touchable word is found
            
            NSString *url = [weakSelf.touchWords objectAtIndex:idx];
            
            if ([[weakSelf.touchWords objectAtIndex:idx] hasPrefix:@"@"]) {
                
                //Twitter account clicked
                if ([weakSelf.delegate respondsToSelector:@selector(twitterAccountClicked:)]) {
                    [weakSelf.delegate twitterAccountClicked:url];
                }
                
            }
            else if ([[weakSelf.touchWords objectAtIndex:idx] hasPrefix:@"#"]) {
                
                //Twitter hashtag clicked
                if ([weakSelf.delegate respondsToSelector:@selector(twitterHashtagClicked:)]) {
                    [weakSelf.delegate twitterHashtagClicked:url];
                }
                
            }
            else if ([[weakSelf.touchWords objectAtIndex:idx] hasPrefix:@"http"]) {
                
                //Twitter hashtag clicked
                if ([weakSelf.delegate respondsToSelector:@selector(websiteClicked:)]) {
                    [weakSelf.delegate websiteClicked:url];
                }
